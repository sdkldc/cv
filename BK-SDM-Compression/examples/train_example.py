...
def create_sample_dataset(num_images=50, image_size=64):
    """간단한 샘플 데이터셋 생성"""
    # 임시 디렉토리 생성
...
def train_example():
    ...
    # 샘플 데이터셋 생성
    print("샘플 데이터셋 생성 중...")
    temp_dir, images_dir = create_sample_dataset(num_images=30, image_size=64)
    ...
    try:
        ...
        model = create_bk_sdm_compression_model(
            ...
            N=64,              # 작은 모델 (예제용)
            M=64,
            diffusion_steps=15,  # 적은 diffusion 단계 (예제용)
            use_pretrained_vae=False,   # 커스텀 VAE 사용
            use_pretrained_unet=False   # 커스텀 U-Net 사용
        ).to(device)
